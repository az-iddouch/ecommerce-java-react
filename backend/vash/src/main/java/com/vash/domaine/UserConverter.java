...
public class UserConverter {
	...
	/*********************************/
	/** with property**/
	public static User toBo2(UserVo userVo) {
		User user = null;
		if (!ObjectUtils.isEmpty(userVo)) {
			user = new User();
			user.setEmail(userVo.getEmail());
	...
	/**with property**/
	public static UserVo toVo2(User user) {
		UserVo userVo = null;
		if (!ObjectUtils.isEmpty(user)) {
			userVo = new UserVo();
			userVo.setEmail(user.getEmail());
	...
	public static List<UserVo> toListVo2(List<User> users) {
		List<UserVo> userVos = new ArrayList<UserVo>();
		if (!ObjectUtils.isEmpty(users)) {
			for (User user : users) {
				userVos.add(toVo2(user));
			}
		}
	...
	public static List<User> toListBo2(List<UserVo> userVos) {
		List<User> users = new ArrayList<User>();
		if (!ObjectUtils.isEmpty(userVos)) {
			for (UserVo userVo : userVos) {
				users.add(toBo2(userVo));
			}
		}
